...
async function main() {
  ...
  // On Vercel, failing to find these usually means the preset or build cwd is wrong.
  if (!fs.existsSync(localOutput)) {
    throw new Error(
      `Expected Vercel output directory to exist, but it was not found: ${localOutput}`,
    );
  }
  if (!fs.existsSync(workspaceFile)) {
    throw new Error(
      `Expected pnpm workspace file to exist, but it was not found: ${workspaceFile}`,
    );
  }
  ...
  fs.cpSync(localOutput, rootOutput, {
    recursive: true,
    verbatimSymlinks: true,
  });
